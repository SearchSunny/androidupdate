...
import com.android_update.utils.NetworkUtil;
...
public class ClinicUpdateManager {
    ...
    private boolean ready;// 安装包是否已经下载完成，避免重复下载
    ...
    public void startUpdate(String url, boolean silent, String version) {
        if (!NetworkUtil.isNetworkUrl(url)) {
            this.mOnUpdateListener.onFailure(new UpdateError(UpdateError.CHECK_URL));
            Log.d(TAG, "url地址错误！");
        } else {
